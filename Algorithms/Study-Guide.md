...
twoSum([...new Set([2, 4, 6, 8, 4, 6, 2, 4, 4, 4, 4, 8, 8])], 10); // => [ [ 2, 8 ], [ 4, 6 ], [ 6, 4 ], [ 8, 2 ] ]
```
...
## Given an array of integers, count number of pairs that have difference k

```javascript
const twoDiff = (arr, diff) => {
  const map = {};
  const pairs = [];
  for (let currentNum of arr) {
    let counterpart = currentNum - diff;
    if (map[counterpart] !== undefined) {
      pairs.push([currentNum, counterpart]);
    }
    map[currentNum] = currentNum;
...
twoDiff([2, 6, 0, 4, 10, 6], 4); => [ [ 6, 2 ], [ 4, 0 ], [ 10, 6 ], [ 6, 2 ] ]
```
...
function countUniqueValues(arr) {
  ...
  for (let j = 1; j < arr.length; j++) {
    if (arr[i] !== arr[j]) {
      // compare arrindex of i and arr index of j (not equal to each other)
...
class LinkedList {
  ...
  reverse() {
    if (!this.head.next) {
      // if next doesn't exist(only 1 item) no need to reverse, just  (!null returns true)
    ...
    while (second) {
      // loop // as long as second has a value and is not null
